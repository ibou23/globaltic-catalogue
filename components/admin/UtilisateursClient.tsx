          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 space-y-4">
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                disabled={isPending}
                className="flex-1 h-10 rounded-xl bg-red-500 text-white text-sm font-bold hover:bg-red-600 transition-colors disabled:opacity-60"
              >
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-slate-400 font-medium mt-1">
              {profiles.length} compte{profiles.length > 1 ? "s" : ""} admin —{" "}
              {profiles.filter((p) => p.isActive).length} actif{profiles.filter((p) => p.isActive).length > 1 ? "s" : ""}
            </p>
          </div>
          <button
            onClick={() => setShowCreateForm(true)}
            className="h-10 px-5 rounded-xl bg-brand-primary text-white text-sm font-bold flex items-center gap-2 hover:bg-brand-primary-dark transition-all"
          >
