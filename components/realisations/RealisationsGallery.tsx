...
import type { FallbackRealisation } from "@/lib/db/fallback";

interface RealisationsGalleryProps {
  realisations: FallbackRealisation[];
  categories: string[];
}
...
export function RealisationsGallery({
  ...
  const [selectedImage, setSelectedImage] =
    React.useState<FallbackRealisation | null>(null);
                ...
                <div className="relative">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-brand-secondary/90 via-brand-secondary/20 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-500 flex flex-col justify-end p-6">
                    <div className="translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                      <p className="text-brand-primary-light text-xs font-black uppercase tracking-widest mb-1">
                        {item.category}
                      </p>
                  ...
                  <div className="space-y-6">
                    <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                      ...
                      <p className="text-brand-secondary font-bold">
                        {selectedImage.client || "Confidentiel"}
                      </p>
                    </div>
                    <p className="text-slate-500 leading-relaxed">
                      Réalisation premium utilisant nos technologies
                      d&apos;impression haute définition. Support durable et
                      rendu des couleurs fidèle à la charte graphique.
                    </p>
